<script>
	 ...
	 */
    function isWithinEventRange(date) {
    ...
    $: {
      ...
      eventsForMonth = getEventsForMonth($currentMonth); // Update events sesuai bulan
    }
        ...
        {#each Array(daysInMonth) as _, i}
          ...
          <div 
            class="day 
              ...
              {isWithinEventRange(new Date($currentMonth.getFullYear(), $currentMonth.getMonth(), i + 1)) ? 'event-range' : ''} 
  ...
  <style>
    ...
    .day.event-range {
      background-color: #A4D4AE; /* Warna untuk rentang event */
      color: white;
    }
