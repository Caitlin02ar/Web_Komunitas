<script lang="ts">
  ...
  import aksi_icon from '$lib/images/aksi.svg';
  ...
  function handleCommunityDetail() {
    ...
    const newUrl = `${currentUrl}/communityDetail`;
    window.location.href = newUrl;
  }

  function handleManageCommunity() {
    const currentUrl = window.location.href;
    const newUrl = `${currentUrl}/Management/CM_Home`;
    window.location.href = newUrl;
  }
...
<div class="community-card">
  <div class="community-header">
    ...
    <div class="actions-menu" on:click={toggleMenu}>
      <img src="{aksi_icon}" alt="Actions" class="actions-icon" />
      {#if showMenu}
  ...
  <div class="action-buttons">
    {#if communityData.admin === 'yes'}
      <button class="manage-community-btn" on:click={handleManageCommunity}>
        Manage Community
      </button>
    {/if}
    {#if communityData.status === 'joined'}
      ...
      >
        <img src="{create}" alt="Create Post" class="create-icon" />
        Create post
      </button>
    {:else}
      {#if communityData.status === 'public'}
        <button class="join-community-btn" on:click={handleJoinDirectly}>Join Community</button>
      {:else if communityData.status === 'private'}
        <button class="join-community-btn" on:click={handleJoinRequest}>Request to Join</button>
      {:else if communityData.status === 'requested'}
        <button class="join-community-btn" disabled>Waiting for Approval</button>
      {/if}
