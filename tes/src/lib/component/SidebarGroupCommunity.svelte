<script lang="ts">
    ...
    function handleChatClick(chatId: number,  hasJoined: boolean) {
      const chat = chats.find(c => c.id === chatId);
      console.log('Clicked chat:', chat); // Periksa data chat
      ...
      if (chat) {
        if (sudahJoin) {
          let currentUrl = window.location.pathname;
          let pathParts = currentUrl.split('/'); // Pisahkan berdasarkan '/'
          let grupId = pathParts[pathParts.indexOf('chat') + 1]; // Ambil ID setelah 'chat'
          const communityPath = currentUrl.split('/chat')[0]; // Extract everything before '/chat'
          const newPath = `${communityPath}/chat/${grupId}/${chatId}`;
          goto(newPath);
        } else if (!sudahJoin) {
    ...
    function handleRequestJoin(chatId: number) {
  
      // Dispatch event to parent
      dispatch('requestJoin', { chatId});
